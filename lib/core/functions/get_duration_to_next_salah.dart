...
Duration getDurationToNextSalah(SalahTimeSuccesRemoteState state) {
  ...
  String nMonth = state.salahList.data![DateTime.now().day - 1].date!.gregorian!
              .month!.number! <
          10
      ? "0${state.salahList.data![DateTime.now().day].date!.gregorian!.month!.number!}"
      : '${state.salahList.data![DateTime.now().day].date!.gregorian!.month!.number!}';
  ...
  DateTime fajrNextDay = DateTime.parse(
      '${state.salahList.data![DateTime.now().day - 1].date!.gregorian!.year!}-$nMonth-${state.salahList.data![DateTime.now().day].date!.gregorian!.day!} ${DateFormat.Hms().format(DateFormat('HH:mm').parse(state.salahList.data![DateTime.now().day].timings!.fajr!.split(' ')[0]))}');
